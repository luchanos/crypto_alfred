...
async def get_referal_system_help(message: types.Message):
    await message.answer(_("Here is our referral system:#######"), reply_markup=main_keyboard())
...
def register_handlers_client(dp: Dispatcher):
    dp.register_message_handler(get_referal_system_help, Text(equals=["Reference ❓", "Справка ❓", "მითითება ❓"]))
    dp.register_message_handler(
        write_to_admin, Text(equals=["Write to admin ✍️", "Написать админу ✍️", "ადმინისთვის მიწერა ✍️"])
    )
    dp.register_message_handler(
    ...
    dp.register_message_handler(
        exchange_rates, Text(equals=["Exchange rates 💲", "Курсы валют 💲", "ვალუტების კურსი 💲"])
    )
    # dp.register_message_handler(
